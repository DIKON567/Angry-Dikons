...
style.configure('W.TButton', font =
               ('AngryBirds', 100, 'underline'),
                foreground = 'red')
fred = Button(frm, fg="red", bg="blue")
ttk.Label(frm, text="Angry Dikons", font='Times 30').grid(column=0, row=0)
ttk.Button(frm, text="Играть!", command=root.destroy, style='W.TButton').grid(column=0, row=1)
ttk.Button(frm, text="Сохранить и выйти", command=sys.exit, style='W.TButton').grid(column=0, row=7)
ttk.Label(frm, text="Alpha v0.0.1", font='Sans').grid(column=8, row=0)
song = '../resources/sounds/angry-birds.ogg'
pygame.init()
...
background2 = pygame.image.load(
    "../resources/images/background2.png").convert_alpha()
...
def unit_vector(v):
    ...
    ub = v[1] / h
    return (ua, ub)
song1 = '../resources/sounds/энгрик.mp3'
...
song2 = '../resources/sounds/dickon1.mp3'
def draw_level_cleared():
    """Draw level cleared"""
    global game_state
    global bonus_score_once
    global score
    level_cleared = bold_font3.render("ТЫ ПОБЕДИЛ!", 1, RED)
...
song3 = '../resources/sounds/dickon2.mp3'
global afds
afds = 0
def draw_level_failed():
    ...
    failed = bold_font3.render("Ты проиграл, лысый!", 10, RED)
...
while running:
    ...
    # Draw score
    score_font = bold_font.render("Счет", 1, WHITE)
    ...
    pygame.display.flip()
    clock.tick(50)
    pygame.display.set_caption("Angry Dikons!")
